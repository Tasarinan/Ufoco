...
function signInWithPopup() {
  return new Promise((resolve, reject) => {
    const authWindow = new remote.BrowserWindow({width: 500, height: 600, show: true});
...
async function fetchAccessTokens(code : string) {
  try {
    const response = await axios.post(GOOGLE_TOKEN_URL, qs.stringify({code, client_id: GOOGLE_CLIENT_ID, client_secret: GOOGLE_CLIENT_SECRET, redirect_uri: GOOGLE_REDIRECT_URI, grant_type: 'authorization_code'}), {
      headers: {
  ...
  for (const event of events) {
    if (event.start && event.end) {
      ...
      try {
        ...
        const result = await store.dispatch(queryTask({
          selector: {
            _id: encodedId
          }
        }));
